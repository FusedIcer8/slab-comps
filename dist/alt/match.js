...
function scoreCandidate(v, slab, anyExactYear, wantLang) {
    ...
    if (slab.year != null && v.year != null) {
        if (v.year === slab.year)
            score += 5;
        else if (!anyExactYear && Math.abs(v.year - slab.year) === 1)
            score += 2;
        else
            score -= 5;
    }
 ...
 */
export function matchValuation(valuations, slab) {
    if (valuations.length === 0)
        return { valuation: null, lowConfidence: false, reasons: [] };
    let pool = valuations;
    if (slab.cardNumber) {
