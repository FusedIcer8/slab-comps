...
function normalizeVarietyText(s: string): string[] {
  return normalizeTokens(s).map(t => {
    if (t === 'first') return '1st'
    if (t === 'ed') return 'edition'
    return t
  })
}
...
function varietyHaystack(v: AltValuation): string[] {
...
function isUnmarkedVariety(v: AltValuation): boolean {
...
function variantMatches(v: AltValuation, wantTokens: string[]): boolean {
...
function setOverlapScore(
  ...
  const extraEditionTokens = candTokens.filter(
    t => EDITION_MARKER_TOKENS.has(t) && !wantSet.has(t) && !exemptTokens.has(t),
  )
...
function scoreCandidate(
  ...
  if (variantWantTokens.length > 0) {
    if (variantMatches(v, variantWantTokens)) score += 2
