import type { AltValuation, CardPop, Game, SlabQuery } from '../types.js'
...
const CATEGORY: Partial<Record<Game, string>> = {
...
async function graphql<T>(operationName: string, query: string, variables: object): Promise<T> {
  ...
  if (!res.ok) throw new Error(`alt graphql ${operationName}: ${res.status} ${res.statusText}`)
  const body = (await res.json()) as { data?: T; errors?: Array<{ message: string }> }
  if (!body.data) throw new Error(`alt graphql ${operationName}: ${body.errors?.[0]?.message ?? 'no data'}`)
  return body.data
}
...
export async function getSearchConfig(): Promise<SearchConfig> {
  if (cachedConfig && cachedConfig.expiresAt - 60 > Date.now() / 1000) return cachedConfig

  interface ConfigResponse {
    serviceConfig: {
      search: {
        universalSearch: {
          clientConfig: { nodes: Array<{ host: string }>; apiKey: string }
          collectionName: string
          expiresAt: number
        }
      }
  ...
  const data = await graphql<ConfigResponse>(
    'SearchServiceConfig',
    `query SearchServiceConfig { serviceConfig { search { universalSearch {
...
export function mapDocToValuation(doc: TypesenseDoc): AltValuation | null {
  if (typeof doc.altValue !== 'number' || doc.altValue <= 0) return null
  return {
...
export async function searchAltValuations(slab: SlabQuery): Promise<AltValuation[]> {
  ...
  const res = await fetch(
    `https://${cfg.host}/multi_search?collection=${cfg.collectionName}&x-typesense-api-key=${encodeURIComponent(cfg.apiKey)}`,
    {
      method: 'POST',
  ...
  if (!res.ok) throw new Error(`alt search: ${res.status} ${res.statusText}`)
  const body = (await res.json()) as {
    results: Array<{ error?: string; hits?: Array<{ document: TypesenseDoc }> }>
  }
  const result = body.results[0]
  if (result.error) throw new Error(`alt search: ${result.error}`)

  const seen = new Set<string>()
  const valuations: AltValuation[] = []
...
export async function getCardPops(assetId: string): Promise<CardPop[]> {
  interface PopsResponse {
    asset: { cardPops: Array<{ gradingCompany: string; gradeNumber: string; count: number }> } | null
  }
  const data = await graphql<PopsResponse>(
    'AssetCardPops',
    'query AssetCardPops($id: ID!) { asset(id: $id) { cardPops { gradingCompany gradeNumber count } } }',
    { id: assetId },
  )
