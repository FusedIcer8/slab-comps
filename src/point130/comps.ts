...
function reprintFamilyQuery(querySetName: string): { original: RegExp; reprint: RegExp } | null {
  ...
  return comps.filter(c => {
    ...
    if (slab.setName) {
      if (reprintFam) {
        ...
        // way). See N1.
        if (!reprintFam.reprint.test(c.title)) return false
      } else {
        ...
        if (!numberSatisfiesSet) {
          if (slab.cardNumber) {
            ...
            if (required.length > 0 && !required.every(tok => titleContainsToken(t, tok))) return false
          }
        }
      }
    }
