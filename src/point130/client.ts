...
export async function search130Point(query: string): Promise<SoldComp[]> {
  let lastError = ''
  for (let attempt = 1; attempt <= 3; attempt++) {
    ...
    if (!res.ok) throw new Error(`130point error: ${res.status} ${res.statusText}`)
...
export function buildQuery(slab: SlabQuery): string {
