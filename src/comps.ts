...
export function computeAltSampleSize(pops: CardPop[], slab: SlabQuery): number {
  const wantedGrade = parseFloat(slab.grade)
  const bucket = pops.find(
    p => p.gradingCompany === slab.grader && parseFloat(p.gradeNumber) === wantedGrade,
  )
...
export function buildRecommendation(
  ...
  options: GetSlabCompsOptions = {},
): Pick<SlabCompsResult, 'recommended' | 'lowConfidence' | 'reasons'> {
  ...
  const disagreementRatio =
    options.disagreementRatio != null && options.disagreementRatio > 1
      ? options.disagreementRatio
      : DEFAULT_DISAGREEMENT_RATIO
  const minSample =
    options.minSample != null && options.minSample >= 1 ? options.minSample : DEFAULT_MIN_SAMPLE

  const altSampleKnown = alt != null && !alt.popsUnavailable && alt.sampleSize != null
  const altThin = altSampleKnown ? (alt!.sampleSize as number) < minSample : false
  const pointThin = point130 ? point130.count < minSample : true

  let recommended: SlabCompsResult['recommended'] = null
  ...
  if (alt?.reasons.length) reasons.push(...alt.reasons)
  if (alt?.popsUnavailable) reasons.push('pops_unavailable')

  if (alt && point130) {
    const hi = Math.max(alt.valuation.altValue, point130.median)
    const lo = Math.min(alt.valuation.altValue, point130.median)
    if (lo > 0 && hi / lo > disagreementRatio) reasons.push('sources_disagree')
  }

  if (recommended) {
    if (recommended.source === 'alt') {
      if (altSampleKnown && (alt!.sampleSize as number) < minSample) reasons.push('thin_sample')
  ...
  if (point130?.yearUnconfirmed) reasons.push('year_unconfirmed')
...
export async function getSlabComps(
  ...
  if (alt?.popsUnavailable) errors.push('alt: population count fetch failed (sample size unknown)')
