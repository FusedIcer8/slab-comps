...
export function computeAltSampleSize(pops: CardPop[], slab: SlabQuery): number | undefined {
...
export function buildRecommendation(
  ...
  if (recommended) {
    if (recommended.source === 'alt') {
      if (alt!.sampleSize != null && alt!.sampleSize < minSample) reasons.push('thin_sample')
...
export async function getSlabComps(
  ...
  if (alt?.popsFetchFailed) errors.push('alt: population count fetch failed (sample size unknown)')
